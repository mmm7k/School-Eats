...
//   const { posts, loading } = useGetCategoryPosts('양식', order);
...
//   await queryClient.prefetchQuery(['posts', 'all', '양식', order], () => fetchPosts('양식', order));
...
export default function WesternPlace({ initialPosts }: { initialPosts: Post[] }): JSX.Element {
  const router = useRouter();
  const [order, setOrder] = useState<OrdKey>((router.query.sort as OrdKey) || 'commentscount');
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const initialPosts = await fetchPosts('양식', order);

...
export const useGetCategoryPosts = (menu: string, category: string) => {
  ...
  const getNextPosts = async () => {
    ...
    try {
      ...
      // }
      if (!lastVisible) {
        q = query(collection(db, menu), where('category', '==', category), limit(4));
      } else {
        q = query(collection(db, menu), where('category', '==', category), limit(4), startAfter(lastVisible));
      }

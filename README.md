...
### 해결 방법
- 서버 사이드 렌더링(SSR) 도입, 이미지 파일을 WebP 변경, Next/Image 컴포넌트의 Lazy Loading으로 문제를 해결했습니다. SSR을 통해 서버에서 초기 HTML을 생성하고, 클라이언트 측에서 최소한의 자바스크립트 작업만으로 페이지를 렌더링할 수 있도록 했습니다. 이를 통해 초기 페이지 로딩 시간이 대폭 단축되었습니다.
